...
public class ChessBoard extends JPanel {
    ...
    public static final int MIN_SPAN = 20; // 最小网格宽度
    public static final int MAX_SPAN = 35; // 最大网格宽度
    public static final int DEFAULT_SPAN = 25; // 默认网格宽度
    ...
    private int span = DEFAULT_SPAN; // 当前网格宽度
    private Image img;
    ...
    public void paintComponent(Graphics g) {
        ...
        for (Chess chess : chessList) {
            ...
            if (chess == chessList.get(chessCount - 1)) {
                ...
                g.drawRect(xPos - (span - 2) / 2, yPos - (span - 2) / 2, span - 2, span - 2);
            }
        }
    }
    ...
    public void adjustPictureSize(String path) {
        // 使用 getResource 获取资源路径
        ...
        if (imagePath != null) {
            ...
            System.err.println("无法找到图像资源: " + path);
        }
        repaint();
    }
    ...
    public void changeWallpaper() {
        ...
        adjustPictureSize(path);
    }

    // 更新 span 值
    public void updateSpan(int newSpan) {
        if (newSpan >= MIN_SPAN && newSpan <= MAX_SPAN) {
            if (this.span != newSpan) {
                this.span = newSpan;
                adjustPictureSize(WALLPAPERS[wallpaperIndex]); // 重新缩放背景图片
                revalidate(); // 重新验证布局
                repaint(); // 重新绘制棋盘
            }
        }
